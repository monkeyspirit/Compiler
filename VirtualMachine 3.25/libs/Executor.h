...
void RDIVf(ExeStack* exe) {
    ...
    float first = pullExeReal(exe);
    if (second==0) {
        printf("ERROR: errore di esecuzione nella divisione tra reali: divisione per zero\n");
        exit(1);
    }
    float result = first / second;
...
void IDIVf(ExeStack* exe) {
    ...
    int first = pullExeInt(exe);
    if (second==0) {
        printf("ERROR: errore di esecuzione nella divisione tra interi: divisione per zero\n");
        exit(1);
    }
    int result = first / second;
